...
describe('saveSessionTurn', () => {
  ...
  it('creates separate files for different dates', () => {
    saveSessionTurn(
      { timestamp: '2026-02-23T10:00:00.000Z', tool: 'Gemini', model: 'g', prompt: 'a', response: 'b' },
      sessionsDir(),
    );
    saveSessionTurn(
      { timestamp: '2026-02-24T10:00:00.000Z', tool: 'Gemini', model: 'g', prompt: 'c', response: 'd' },
      sessionsDir(),
    );
...
describe('loadRecentTurns', () => {
  ...
  it('returns saved turns in order (oldest first)', () => {
    const s = sessionsDir();
    saveSessionTurn({ timestamp: '2026-02-24T09:00:00.000Z', tool: 'Gemini', model: 'g', prompt: 'first', response: 'r1' }, s);
    saveSessionTurn({ timestamp: '2026-02-24T10:00:00.000Z', tool: 'Claude', model: 'c', prompt: 'second', response: 'r2' }, s);
  ...
  it('respects the limit parameter', () => {
    const s = sessionsDir();
    for (let i = 0; i < 5; i++) {
      saveSessionTurn({ timestamp: `2026-02-24T0${i}:00:00.000Z`, tool: 'Gemini', model: 'g', prompt: `p${i}`, response: `r${i}` }, s);
    }
  ...
  it('returns turns with all required fields', () => {
    const s = sessionsDir();
    saveSessionTurn({ timestamp: '2026-02-24T10:00:00.000Z', tool: 'Codex', model: 'gpt-4o', prompt: 'hi', response: 'hello' }, s);
    const turns = loadRecentTurns(10, s);
    expect(turns[0]!.timestamp).toBe('2026-02-24T10:00:00.000Z');
    expect(turns[0]!.tool).toBe('Codex');
  ...
  it('skips malformed lines without throwing', () => {
    ...
    saveSessionTurn({ timestamp: '2026-02-24T10:00:00.000Z', tool: 'Gemini', model: 'g', prompt: 'good', response: 'ok' }, s);
    const turns = loadRecentTurns(10, s);
